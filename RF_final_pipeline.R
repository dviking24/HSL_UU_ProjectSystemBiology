...
features_and_labels <- function(split_data) {
  ...
  y_train <- factor(train_matrix$PregnancyStatus,
                    levels = c("NOT_PREGNANT", "PREGNANT"))
  ...
  y_test <- factor(test_matrix$PregnancyStatus,
                   levels = c("NOT_PREGNANT", "PREGNANT"))
...
rf_model_running <- function(X_train, y_train, seed) {
...
varImp_function <- function(model, nb_of_feat, X_train, X_test, y_train, y_test, seed) {
  ...
  rf_Imp <- rf_model_running(X_train_small, y_train, seed)
  
  pred_list <- evaluate_predictions(
    rf_Imp,
    X_train_small,
    y_train,
    X_test_small,
...
pipeline <- function(matrix, train_sample_amount, test_sample_amount, seeds_to_run, name) {
  ...
  base_dir <- file.path("rf_results_one", name)
  ...
  results <- lapply(seeds_to_run, function(seed) {
    ...
    rf_model <- rf_model_running(X_train, y_train, seed)
    basic_pred_list <- evaluate_predictions(rf_model, X_train, y_train, X_test, y_test)
    ...
    # SHAP best_model
    best_model_file <- file.path(seed_dir, "best_model.rds")
    best_X_train_file <- file.path(seed_dir, "X_train.csv")
    
    best_model <- readRDS(best_model_file)
    best_X_train <- read.csv(best_X_train_file, check.names = FALSE)
    ...
    # }
    # shap_results(shap_values$S, colnames(best_X_train), seed_dir)
...
full_pipeline <- function() {
  ...
  if (WHICH == "ALL" | WHICH == "VIVO_VITRO_lig-embryo") {
    ...
    pipeline(vitro, VITRO_TRAIN_A, VITRO_TEST_A, SEEDS_TO_RUN, vivo_n)
